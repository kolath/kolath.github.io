document.addEventListener('DOMContentLoaded', function () {
  ...
  // failed: optional "2 failed" text shown on right
  var QUEUE = [
    { name: 'Publish menu',              taskKey: 'progress',  state: 'progress',  count: '12/50' },
  ...
  function queueTaskDescription(task) {
    ...
    if (task.state === 'progress') {
      return task.count ? task.count + ' locations published' : 'Publishing now';
    }
    if (task.state === 'completed') return 'Published';
    if (task.state === 'cancelled') return 'Canceled';
    if (task.failed) return task.failed;
    return 'Queued';
  }
  ...
  function queueRender() {
    ...
    QUEUE.forEach(function(task) {
      ...
      var failedHtml = task.failed
        ? '<span class="pq-task__failed">' + task.failed + '</span>'
        : '';
  ...
  window.pqAddPublishMenuTask = function() {
    // New task starts as pending (only 1 can be in-progress at a time)
    var hasActive = QUEUE.some(function(t) { return t.state === 'progress'; });
    QUEUE.push({
      name: 'Publish menu',
      taskKey: 'progress',
      state: hasActive ? 'pending' : 'progress',
      count: hasActive ? null : '0/25',
    });

    queueRender();

    // Show card expanded, hide pill
