// Modifier Groups table interactions: select-all checkbox + column toggle + sorting + bulk actions
document.addEventListener('DOMContentLoaded', () => {
  const table = document.getElementById('mg-data-table');
  if (!table) return;

  // ── Select-all checkbox sync + bulk actions ──
  const selectAll = document.getElementById('mg-select-all');
  const rowCheckboxes = () => table.querySelectorAll('tbody .mg-table-checkbox');
  const bulkBar = document.getElementById('mg-bulk-action-bar');
  const bulkCount = document.getElementById('mg-bulk-action-count');
  const bulkDismiss = document.getElementById('mg-bulk-action-dismiss');
  ...
  if (selectAll) {
    ...
    table.querySelector('tbody').addEventListener('change', (e) => {
      if (!e.target.classList.contains('mg-table-checkbox')) return;
  ...
  // ── Column toggle ──
  const toggleBtn = document.getElementById('mg-column-toggle-btn');
  const dropdown = document.getElementById('mg-column-toggle-dropdown');
  const listContainer = document.getElementById('mg-column-toggle-list');
  ...
  if (toggleBtn && dropdown) {
    ...
    headers.forEach((th, index) => {
      if (th.classList.contains('col-checkbox') || th.classList.contains('col-mg-actions')) return;
    ...
    document.addEventListener('click', (e) => {
      if (!e.target.closest('#mg-column-toggle')) {
